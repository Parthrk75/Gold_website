...
//   return (
//     <MDXProvider>
//       <main className="min-h-screen bg-gradient-to-b from-yellow-50 via-white to-white">
//         {activePost === null ? (
//           <>
//             {featuredPost && (
...
//                     <span className="inline-block px-4 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-semibold">Featured</span>
//                     <h1 className="text-4xl font-extrabold text-gray-900 sm:text-5xl">{featuredPost.title}</h1>
//                     <p className="text-lg text-gray-600">{featuredPost.summary}</p>
...
//                     </div>
//                     <button onClick={() => setActivePost(featuredPost.id)} className="inline-flex items-center gap-2 px-5 py-3 bg-yellow-500 text-white rounded-full hover:bg-yellow-600 transition">
//                       Read More <ChevronRight size={20} />
//                     </button>
//                   </div>
//                   <img src={featuredPost.imageUrl} alt={featuredPost.title} className="w-full h-96 object-cover rounded-3xl shadow-2xl" />
//                 </div>
//               </section>
//             )}
...
//                   <div key={post.id} onClick={() => setActivePost(post.id)} className="cursor-pointer group bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm hover:shadow-lg transition">
//                     <div className="relative h-56 overflow-hidden">
//                       <img src={post.imageUrl} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
//                       <span className="absolute top-4 left-4 bg-white/90 text-yellow-700 text-xs font-semibold px-3 py-1 rounded-full shadow">{post.category}</span>
//                     </div>
//                     <div className="p-6 space-y-4">
//                       <h2 className="text-xl font-bold text-gray-800 group-hover:text-yellow-600">{post.title}</h2>
...
//               <button onClick={() => setActivePost(null)} className="flex items-center gap-2 text-blue-600 text-sm mb-8 hover:underline">
