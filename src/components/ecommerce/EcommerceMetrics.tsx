...
export const EcommerceMetrics = () => {
  ...
  return (
    <div className="">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-3">
        {["24K", "22K", "18K"].map((karat) => {
          ...
          return (
            <div
              key={karat}
              className={`rounded-xl border border-gray-200 p-6 dark:border-gray-800 shadow-md transition-all ${shadowColor} bg-white dark:bg-gray-900`}
            >
              <div className="flex justify-between items-center">
                <span className="text-lg font-semibold text-transparent bg-gradient-to-r from-yellow-500 via-amber-500 to-yellow-300 bg-clip-text">
                  {karat} Gold Price
                </span>
                {priceChange !== 0 && (
                  <span className={`flex items-center gap-1 text-lg font-semibold ${isIncreased ? "text-green-600" : "text-red-600"}`}>
                    {isIncreased ? <TrendingUp className="h-5 w-5" /> : <TrendingDown className="h-5 w-5" />}
                  </span>
                )}
              </div>
              <h4 className="mt-3 text-3xl font-bold text-gray-900 dark:text-white">${price.toFixed(2)}</h4>
            </div>
          );
        })}
      </div>
      ...
      {/* Alert Banner */}
      <div className="w-full rounded-xl bg-yellow-100 border-l-4 border-yellow-500 mt-8 p-5 dark:bg-yellow-900 dark:border-yellow-600">
        <div className="flex items-start gap-3">
          <AlertTriangle className="h-5 w-5 text-yellow-600 dark:text-yellow-400 mt-0.5" />
          <div>
            <h3 className="font-semibold text-yellow-800 dark:text-white">Gold Market Closed on Weekends</h3>
            <p className="text-yellow-700 dark:text-white mt-3">
              Gold prices stay unchanged on weekends as the market is closed.
            </p>
          </div>
        </div>
      </div>
